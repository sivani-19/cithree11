...
<?php $id= $this->session->userdata('userid');?>
    <?php echo form_open('E_C/Login_C/update_profile?id='.$id);?>
  <form action="<?php echo base_url('E_C/Login_C/update_profile?id='.$id);?>" method="post">
...
@media screen and (max-width:478px) {
  .mb-3 input{padding:0.8rem 0.5rem;width:80vw;position: relative;margin-top:0.8rem;border-radius: 5px;font-size: 1.2rem;}
  .mb-3 label{font-size: 1.8rem;}
  .P_S i{position: absolute; right:10px;top:2.9rem;cursor: pointer;z-index: 2;}
}
@media screen and (min-width:478px) and (max-width:1024px) {
  .mb-3 input{padding:0.8rem 0.5rem;width:80vw;position: relative;margin-top:0.8rem;border-radius: 5px;font-size: 1.2rem;}
  .mb-3 label{font-size: 1.8rem;}
  .P_S i{position: absolute; right:10px;top:2.9rem;cursor: pointer;z-index: 2;}
  .btn{position:relative;margin-top:2rem;color:white;background-color: black;border-radius: 5px;border:none;padding: 10px;font-size: 1.5rem;text-align: center;}
